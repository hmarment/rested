...
def test_client_multiple_integrations(client_with_multiple_integrations):
    ...
    assert hasattr(client_with_multiple_integrations, 'myapi1') \
        and isinstance(
            client_with_multiple_integrations.myapi1, Integration) \
        and hasattr(client_with_multiple_integrations, 'myapi2') \
        and isinstance(
            client_with_multiple_integrations.myapi2, Integration) \
        and hasattr(client_with_multiple_integrations, 'myapi3') \
        and isinstance(
            client_with_multiple_integrations.myapi3, Integration)


def test_client_new(client, integration):

    client.integrations.new(name=integration.name,
